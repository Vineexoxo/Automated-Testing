...
export async function POST(request: Request) {
    try {
        ...
        if (!userId) {
            console.warn("[USER_UPDATE] Unauthorized: User is not authenticated");
        ...
        if (!authUser || authUser.role !== 'ADMIN') {
            console.warn("[USER_UPDATE] Unauthorized: User does not have ADMIN role");
            return new NextResponse("Unauthorized", { status: 401 });
        }
        ...
        if (!email) {
            console.warn("[USER_UPDATE] Bad Request: Email is required");
